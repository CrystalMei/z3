...
template<typename Ext>
bool theory_diff_logic_weak<Ext>::internalize_atom(app * n, bool gate_ctx, bool not_flag) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: internalize_atom with gate_ctx(" << gate_ctx << ") and not_flag(" << not_flag << "): " << mk_pp(n, m) << "\n";);
    if (!m_consistent)
        return false;
    ...
    IF_VERBOSE(5, verbose_stream() << "W-DL: internalize_atom: LHS = " << mk_pp(lhs, m) << "\n";);
    app * rhs = to_app(n->get_arg(1));
    IF_VERBOSE(5, verbose_stream() << "W-DL: internalize_atom: RHS = " << mk_pp(rhs, m) << "\n";);
    ...
    if (target == 0 || source == 0) {
        ...
        IF_VERBOSE(5, verbose_stream() << "W-DL: <NOT> edge\n";);
        if (k > numeral(0)) {
            IF_VERBOSE(5, verbose_stream() << "W-DL: <NOT> edge with weight > 0\n";);
            k.neg();
            if (m_util.is_int(lhs)) {
                SASSERT(k.is_int());
                k -= numeral(1);
        ...
        IF_VERBOSE(5, verbose_stream() << "W-DL: edge\n";);
        if (k >= numeral(-1)) {
            IF_VERBOSE(5, verbose_stream() << "W-DL: edge with weight >= -1\n";);
            edge_id pos = m_graph.add_edge(source, target, k, l);
            k.neg();
            if (m_util.is_int(lhs)) {
                SASSERT(k.is_int());
            ...
            edge_id neg = m_graph.add_edge(target, source, k, ~l);
            // edge_id neg = m_graph.add_edge(target, source, numeral(0), ~l);
            atom * a = alloc(atom, bv, pos, neg);
            m_atoms.push_back(a);
            m_bool_var2atom.insert(bv, a);
...
void theory_diff_logic_weak<Ext>::internalize_eq_eh(app * atom, bool_var v) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: internalize_eq_eh: " << mk_pp(atom, m) << "\n";);
...
bool theory_diff_logic_weak<Ext>::is_sign(expr* n, bool& sign) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: is_sign: " << mk_pp(n, m) << "\n";);
    rational r;
    expr* x;
...
bool theory_diff_logic_weak<Ext>::is_negative(app* n, app*& m) { 
    IF_VERBOSE(5, verbose_stream() << "W-DL: is_negative: " << mk_pp(n, get_manager()) << "\n";);
    expr* a0, *a1, *a2;
    rational r;
...
bool theory_diff_logic_weak<Ext>::is_offset(app* n, app*& v, app*& offset, rational& r) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: is_offset: " << mk_pp(n, m) << "\n"; );
    if (!m_util.is_add(n)) {
        return false;
    }
...
theory_var theory_diff_logic_weak<Ext>::mk_num(app* n, rational const& r) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: mk_num: " << mk_pp(n, m) << "\n"; );
...
void theory_diff_logic_weak<Ext>::set_sort(expr* n) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: set_sort: " << mk_pp(n, m) << "\n"; );
...
theory_var theory_diff_logic_weak<Ext>::mk_var(app* n) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: mk_var starts: " << mk_pp(n, m) << "\n";);
    enode* e = nullptr;
    theory_var v = null_theory_var;
    ...
    for (unsigned i = 0; i < num_edges; ++i) {
        ...
        if (n_x < n_y + n_c && k_x > k_y + k_c) {
            ...
            if (new_delta < m_delta) {
                IF_VERBOSE(5, verbose_stream() << "new delta: " << new_delta << "\n";);
...
void theory_diff_logic_weak<Ext>::new_eq_or_diseq(bool is_eq, theory_var v1, theory_var v2, justification& eq_just) {
    ...
    if (s == t) {
        ...
        IF_VERBOSE(5, verbose_stream() << v1 << " .. " << v2 << "\n" << mk_pp(eq.get(), m) << "\n";);
        TRACE("diff_logic", 
              tout << v1 << " .. " << v2 << "\n";
              tout << mk_pp(eq.get(), m) <<"\n";);
...
theory_var theory_diff_logic_weak<Ext>::add_objective(app* term) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: add_objective: " << mk_pp(term, m) << "\n";);
...
bool theory_diff_logic_weak<Ext>::internalize_objective(expr * n, rational const& m, rational& q, objective_term & objective) {
    IF_VERBOSE(5, verbose_stream() << "W-DL: internalize_objective\n" << mk_pp(n, get_manager()) << "\n";);
